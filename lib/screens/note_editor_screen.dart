...
class _NoteEditorScreenState extends State<NoteEditorScreen>
  ...
  final List<String> _colors = ['yellow', 'blue', 'green', 'purple', 'pink', 'orange'];
  final List<String> _categories = ['Personal', 'Work', 'Ideas', 'Shopping', 'Travel'];
    ...
    return Consumer<AppProvider>(
      builder: (context, provider, child) {
        final noteGradient = provider.isDarkMode 
            ? AppTheme.darkNoteColors[_selectedColor] ?? AppTheme.darkNoteColors['yellow']!
            : AppTheme.noteColors[_selectedColor] ?? AppTheme.noteColors['yellow']!;
            ...
            child: SafeArea(
              child: Column(
                children: [
                  ...
                  // Editor content
                  Expanded(
                    child: _buildEditorContent(noteGradient, provider.isDarkMode),
                  ),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              provider.currentNote != null ? 'Edit Note' : 'Create Note',
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          // Content field
          Expanded(
            child: _buildContentField(noteGradient, isDarkMode),
          ),
        ],
      ),
    );
  ...
  Widget _buildTitleField(LinearGradient noteGradient, bool isDarkMode) {
    return Container(
      decoration: BoxDecoration(
        gradient: noteGradient.colors.map((c) => c.withOpacity(0.1)).toList().length >= 2
            ? LinearGradient(
                colors: noteGradient.colors.map((c) => c.withOpacity(0.1)).toList(),
              )
            : null,
